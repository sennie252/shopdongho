  ...
  if (username && password) {
    ...
    if (customer) {
      if (customer.active === 1) {
        ...
        res.json({ success: false, message: 'Tài khoản đã bị vô hiệu hóa' });
      }
    } else {
      res.json({ success: false, message: 'Không đúng tài khoản hoặc mật khẩu' });
    }
  } else {
...
router.post('/signup', async function (req, res) {
  ...
  if (dbCust) {
    res.json({ success: false, message: 'Tên tài khoản hoặc email đã tồn tại' });
  } else {
    ...
    if (result) {
      ...
      res.json({ success: false, message: 'Đăng kí không thành công' });
    }
  }
});
// myprofile
...
router.post('/checkout', JwtUtil.checkToken, async function (req, res) {
  ...
  const order = { cdate: now, total: total, status: 'ĐANG CHỜ', customer: customer, items: items };
